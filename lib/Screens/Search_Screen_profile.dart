    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            onPressed: () {},
            icon: Icon(Icons.notifications_none, size: 34),
          )
        ],
      ),
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('Posts').where('isGlobalOptionEnabled',isEqualTo: true).snapshots(),
        builder: (context, AsyncSnapshot<QuerySnapshot<Map<String, dynamic>>> snapshot) {
          ...
          List<DocumentSnapshot<Map<String, dynamic>>> posts = snapshot.data!.docs;

          posts.sort((a, b) {
            if (a['uid'] == widget.uid && a['username'] == widget.username) {
              return -1; // Put post with specific uid and username at the beginning
            } else if (b['uid'] == widget.uid && b['username'] == widget.username) {
              return 1;
            } else {
              return 0;
            }
