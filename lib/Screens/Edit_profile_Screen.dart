...
import 'package:newapp/resources/Storage_methods.dart';
  ...
  void Updatetask()async {
    if (_image != null) {
      ...
      String photourl = await StorageMethods().UploadImagetoStorage(
          "ProfilePics", _image!, false,storeduid);
      ...
      if(namecontroller.text.isNotEmpty&&BioController.text.isEmpty&&Catecontroller.text.isEmpty) {
        await _firestore.collection('users').doc(myusername).update({
          'name': namecontroller.text,
        });
      }
      else if(namecontroller.text.isEmpty&&BioController.text.isNotEmpty&&Catecontroller.text.isEmpty) {
        await _firestore.collection('users').doc(myusername).update({
          'Bio': BioController.text,
        });
      }
     else  if(namecontroller.text.isEmpty&&BioController.text.isEmpty&&Catecontroller.text.isNotEmpty) {
        await _firestore.collection('users').doc(myusername).update({
          'Category': Catecontroller.text,
        });
      }
     else if(namecontroller.text.isNotEmpty&&BioController.text.isNotEmpty&&Catecontroller.text.isNotEmpty){
        await _firestore.collection('users').doc(myusername).update({
          'name': namecontroller.text,
          'Bio': BioController.text,
          'Category': Catecontroller.text,
