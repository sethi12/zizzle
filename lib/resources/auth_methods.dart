...
import 'package:newapp/model/user.dart' as model;
...
class Authmethods {
      ...
      if (username.isNotEmpty || email.isNotEmpty || password.isNotEmpty) {
        ...
        if (existinguser.exists) {
          ...
          String photourl = await StorageMethods()
              .UploadImagetoStorage("ProfilePics", file, false,_auth.currentUser!.uid);
  ...
  Future<String> LoginUserwithUsername({required usernamee, required passwordd}) async {
    String ress = "No user Found";
    try {
      String username = usernamee.toString().trim();
      String password = passwordd.toString().trim();

      var existingUser = await _firestore.collection("users").doc(username).get();
        print(existingUser);
      if (existingUser.exists) {
        ...
        if (password == storedPassword) {
          await _auth.signInWithEmailAndPassword(email: storedemail, password: storedPassword);
